...
- @b /d435/depth/color/points [sensor_msgs::PointCloud2] data from D435 camera

Publishes:

- @b d435_obstacles [sensor_msgs::PointCloud2] grid cells that
  contain an obstacle

- @b d435_clear [sensor_msgs::PointCloud2] grid cells with no
  obstacles
...
#define MAX(x, y) ((x) > (y) ? (x) : (y))
    ...
    {
        // get parameters using private node handl
        priv_nh.param("cell_size", m_per_cell_, 0.05);
        ...
        obstacle_publisher_ = node.advertise<VPointCloud>("d435_obstacles", 1);
        clear_publisher_ = node.advertise<VPointCloud>("d435_clear", 1);
        
        velodyne_scan_ = node.subscribe("/d435/depth/color/points", 10,
